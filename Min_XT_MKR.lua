jj
game:GetService("CoreGui"):FindFirstChild("MUKURO GUI") then
...
function DiscordLib:Window(text,mainclr,cls)
	...
	local imizeBtn = Instance.new("TextButton")
	local imizeIcon = Instance.new("ImageLabel")
	...
	function ServerHold:Server(text, img)
		...
		function ChannelHold:Channel(text,ico)
			...
			function ChannelContent:Slider(text, , max, start, callback)
				local SliderFunc = {}
				...
				ValueLabel.Text = tostring(start and math.floor((start / max) * (max - ) + ) or 0)
				...
				local function move(input)
					...
					Zip.Position = pos
					local value = math.floor(((pos.X.Scale * max) / max) * (max - ) + )
				...
				function SliderFunc:Change(tochange)
					...
					ValueLabel.Text = tostring(tochange and math.floor((tochange / max) * (max - ) + ) or 0)
...
function UpdateTime()
    ...
    local ute = math.floor(GameTime/(60^1))%60
    local Second = math.floor(GameTime/(60^0))%60
    Time:Refresh("Hour : "..Hour.." ute : "..ute.." Second : "..Second)
end
...
local RaceV4 = serv:Channel("Tộc V4", "http://www.roblox.com/asset/?id=7040391851")
    ...
    RaceV4:Button("Tp đến cây",function()
      ...
      RaceV4:Button("Tự động ải thiên thần",function(t)
              ...
              game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Workspace").Map.kTrial.Ceiling.CFrame * CFrame.new(0,-5,0)
              end)
					...
					pcall(function()
						for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
							if v.Name == Ms then
								repeat game:GetService("RunService").Heartbeat:wait()
									if string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, NameMon) then
										Health = v.Humanoid.MaxHealth * HealthPersen/100
										if v.Humanoid.Health <= Health then
...
function CheckBossQuest()
    if Old_World then
        ...
        elseif SelectBoss == "Chief Warden [Lv. 200] [Boss]" then
            ...
            CFrameQBoss = CFrame.new(4853.283203125, 5.6783537864685, 745.13970947266)
            CFrameBoss = CFrame.new(5020.9438476563, 88.67887878418, 756.89392089844)
        elseif SelectBoss == "Flago [Lv. 225] [Boss]" then
            BossMon = "Flago [Lv. 225] [Boss]"
            NameQuestBoss = "ImpelQuest"
            QuestLvBoss = 3
            RewardBoss = "Reward:\n$15,000\n1,300,000 Exp."
            CFrameQBoss = CFrame.new(4853.283203125, 5.6783537864685, 745.13970947266)
